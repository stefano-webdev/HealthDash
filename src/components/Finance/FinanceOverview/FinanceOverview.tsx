...
function FinanceOverview() {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < new Date().getMonth(); i++) {
            const month = savedData.finance?.monthlyCashFlow[months[i]];
            const balance = month ? month.revenue - month.expenses : 0;
            currentBalance += balance;
        }

        setCashFlow({
            revenue: savedData.finance?.monthlyCashFlow[months[new Date().getMonth() - 1]].revenue ?? 0,
            expenses: savedData.finance?.monthlyCashFlow[months[new Date().getMonth() - 1]].expenses ?? 0,
